...
const blank = { title: '', description: '', category_id: '', created_by: '', department: '', location: '', priority: 'normal' }
...
export default function TicketsPage() {
  ...
  return (
    <div>
      <h4>Hospital Support Requests</h4>
      ...
      <form onSubmit={handleSubmit} className="row g-2 mb-3">
        ...
        <div className="col-md-2">
          <input className="form-control form-control-sm" placeholder="Department" required minLength={2}
            value={form.department} onChange={e => setForm({ ...form, department: e.target.value })} />
        </div>
        <div className="col-md-2">
          <input className="form-control form-control-sm" placeholder="Ward / room / location" required minLength={2}
        ...
        <div className="col-md-2">
          <button className="btn btn-sm btn-primary w-100" type="submit">Submit Request</button>
        </div>
      ...
      {loading ? <p>Loading…</p> : (
        <table className="table table-sm table-bordered table-hover">
          <thead className="table-dark">
            <tr><th>Request</th><th>Type</th><th>Department</th><th>Location</th><th>Priority</th><th>Status</th><th>Created</th><th></th></tr>
          </thead>
          <tbody>
            {tickets.length === 0 && <tr><td colSpan={8} className="text-center text-muted">No support requests.</td></tr>}
            {tickets.map(t => (
              <tr key={t.id} style={{ cursor: 'pointer' }} onClick={() => navigate(`/tickets/${t.id}`)}>
                ...
                <td>{t.department || '—'}</td>
                <td>{t.location || '—'}</td>
                <td><span className={`badge bg-${t.priority === 'urgent' ? 'danger' : t.priority === 'high' ? 'warning text-dark' : 'secondary'}`}>{t.priority || 'normal'}</span></td>
